...
// TestCRIImagePullMirrorAuth exercises how a CRI PullImageRequest's auth is
// distributed across the registry named by the image reference and its
// configured mirror.
//
// The primary registry must always see the request auth. The mirror must see
// it only when the request auth is ServerAddress-scoped (the shape kubelet
// builds from an imagePullSecret) — an unscoped request auth stays scoped to
// the primary registry and is withheld from mirrors.
func TestCRIImagePullMirrorAuth(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "UnscopedAuthWithheldFromMirror", scopeToPrimary: false, forwardToMirror: false, mirrorSeesCreds: false},
		{name: "UnscopedAuthReachesMirrorWhenForwardEnabled", scopeToPrimary: false, forwardToMirror: true, mirrorSeesCreds: true},
		{name: "ServerAddressScopedAuthWithheldByDefault", scopeToPrimary: true, forwardToMirror: false, mirrorSeesCreds: false},
		{name: "ServerAddressScopedAuthReachesMirrorWhenForwardEnabled", scopeToPrimary: true, forwardToMirror: true, mirrorSeesCreds: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			t.Run("LocalPull", func(t *testing.T) {
				t.Parallel()
				testCRIImagePullMirrorAuth(t, tc.scopeToPrimary, tc.forwardToMirror, tc.mirrorSeesCreds, true)
			})
			t.Run("TransferService", func(t *testing.T) {
				t.Parallel()
				testCRIImagePullMirrorAuth(t, tc.scopeToPrimary, tc.forwardToMirror, tc.mirrorSeesCreds, false)
			})
		})
	}
}

func testCRIImagePullMirrorAuth(t *testing.T, scopeToPrimary, forwardToMirror, mirrorSeesCreds, useLocal bool) {
	const (
		testUser   = "user"
		testPasswd = "passwd"
	)
	...
	registryCfg := criconfig.Registry{
		...
		ForwardRequestAuthToMirrors: forwardToMirror,
	}
	...
	if scopeToPrimary {
		// Mirrors what kubelet builds from a docker config `auths` entry
		// keyed on the primary registry.
	...
	if mirrorSeesCreds {
		assert.Contains(t, mirrorSeen(), expectedAuth,
			"mirror should see the ServerAddress-scoped request auth")
	} else {
		assert.NotContains(t, mirrorSeen(), expectedAuth,
			"mirror must not see an unscoped request auth")
	}
}
