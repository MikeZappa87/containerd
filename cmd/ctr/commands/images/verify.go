...
blobs with a confirmed size or digest mismatch are removed after verification.`,
	...
	Action: func(cliContext *cli.Context) error {
		...
		matcher, err := verificationMatcher(cliContext)
		if err != nil {
			return err
		}
		failures := make(map[digest.Digest]verifyFailure)
		for _, image := range imageList {
			for _, failure := range verifyImage(ctx, contentStore, image.Target(), matcher) {
...
func verificationMatcher(cliContext *cli.Context) (platforms.MatchComparer, error) {
	if cliContext.Bool("all-platforms") {
		return nil, nil
	}
	if values := cliContext.StringSlice("platform"); len(values) > 0 {
		parsed, err := platforms.ParseAll(values)
		if err != nil {
			return nil, err
		}
		return platforms.Ordered(parsed...), nil
	}
	return platforms.MatchComparer(platforms.Default()), nil
}
...
func verifyImage(ctx context.Context, provider content.Provider, target ocispec.Descriptor, matcher platforms.MatchComparer) []verifyFailure {
	...
	for len(pending) > 0 {
		...
		if matcher != nil {
			childrenHandler = coreimages.LimitManifests(coreimages.FilterPlatforms(childrenHandler, matcher), matcher, 1)
		}
