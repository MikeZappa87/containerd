...
and re-pull it. Unreadable content is never removed automatically.`,
	...
	Action: func(cliContext *cli.Context) error {
		...
		failures := verifyImage(ctx, client.ContentStore(), image.Target(), matcher)
		if len(failures) == 0 {
		...
		if err := client.ImageService().Delete(ctx, ref, coreimages.SynchronousDelete()); err != nil {
		...
		if failures = verifyImage(ctx, client.ContentStore(), image.Target(), matcher); len(failures) != 0 {
